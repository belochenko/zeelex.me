...
import { NodeData, EdgeData, ChangelogEntry } from './types';
import { TRL_COLOR, TRL_LABELS, CHAPTER_COLOR, CHAPTER_LABEL } from './constants';
import { CivStackSidePanel } from './components/CivStackSidePanel';
...
export default function CivStackClient() {
  ...
  // Crosshairs & Native Mouse Tracking
  const cxLayerRef = useRef<HTMLDivElement>(null);
  const cxXRef = useRef<HTMLDivElement>(null);
  const cxYRef = useRef<HTMLDivElement>(null);
  
  const tipNodeRef = useRef<string | null>(null);
  const tipVisRef = useRef<boolean>(false);
  useEffect(() => {
    tipNodeRef.current = tip.nodeId;
    tipVisRef.current = tip.visible;
  }, [tip]);

  useEffect(() => {
    const updateMouse = (e: MouseEvent) => {
      if (tipVisRef.current && tipNodeRef.current) return; // let React render handle locked aim
      if (cxXRef.current) cxXRef.current.style.top = `${e.clientY}px`;
      if (cxYRef.current) cxYRef.current.style.left = `${e.clientX}px`;
      if (cxLayerRef.current) {
        cxLayerRef.current.style.webkitMaskImage = 'none';
        cxLayerRef.current.style.opacity = '0.8';
      }
    };
    
    const hideMouse = () => { if (cxLayerRef.current) cxLayerRef.current.style.opacity = '0'; };
    ...
    <>
      ...
      {(() => {
        ...
        if (tip.visible && tip.nodeId && capMap[tip.nodeId]) {
          ...
          finalX = node.gx * gTx.s + gTx.x + offX;
          finalY = node.gy * gTx.s + gTx.y + offY;
          rMask = (node.nodeR + 12) * gTx.s;
          isLocked = true;
        }

        const maskStyle = isLocked
          ? { WebkitMaskImage: `radial-gradient(circle at ${finalX}px ${finalY}px, transparent ${rMask}px, black ${rMask}px)` }
          : {};

        return (
          <div className="cs-crosshair-layer" ref={cxLayerRef} style={maskStyle}>
            <div className="cs-crosshair-x" ref={cxXRef} style={isLocked ? { top: finalY } : undefined} />
            <div className="cs-crosshair-y" ref={cxYRef} style={isLocked ? { left: finalX } : undefined} />
          </div>
        );
      })()}
